...
namespace MinecraftServerSoftware.Operations
{
    public class OperationManager
    {
        ...
        static Servers.ServerJars ServerJars = new Servers.ServerJars();
        ...
        public static void CreateServer(string servername)
        {
            ...
            if ((CommandOrganizer.useSpigotOnly || (!Paper.DoesVersionExist(chosenversion) && !CommandOrganizer.useBukkitOnly && !CommandOrganizer.useVanillaOnly && !CommandOrganizer.usePaperOnly)) && ServerJars.InstallSpigotJar(chosenversion, servername)) { }
            else if ((CommandOrganizer.useBukkitOnly || (!Paper.DoesVersionExist(chosenversion) && !CommandOrganizer.useSpigotOnly && !CommandOrganizer.useVanillaOnly && !CommandOrganizer.usePaperOnly)) && ServerJars.InstallBukkitJar(chosenversion, servername)) { }
            else if ((CommandOrganizer.useVanillaOnly || (!Paper.DoesVersionExist(chosenversion) && !CommandOrganizer.useBukkitOnly && !CommandOrganizer.useSpigotOnly && !CommandOrganizer.usePaperOnly)) && ServerJars.InstallVanillaJar(chosenversion, servername)) { }
            ...
            // creating start.bat
            Screen.Print("\r::How much dedicated RAM (in gigs) would you like the server to have? (default is 2G) >> ", ConsoleColor.Green);
            ...
            File.WriteAllText("./server/" + servername + "/start.bat", "java -Xmx" + dedicatedram + "G -Xms1024M -jar server.jar nogui");
            Screen.Print("\r     -Assigned dedicated RAM        ", ConsoleColor.Green);
            
            //agreeing to EULA
            Screen.Print("\n::Do you agree to the EULA? Type `agree` to confirm you agree to the EULA >> ", ConsoleColor.Green);
        ...
        public static void DeleteServer(string servername)
        {
            ...
            {
                Screen.PrintLn("     -You cannot delete the server while it is running, to stop the server type 'stop' into the server console", ConsoleColor.Red); 
                Environment.Exit(0);
            }
            Directory.Delete(@".\server\" + servername, true);
        ...
        public static void ListServers()
        {
            ...
            foreach (string folder in Directory.GetDirectories(@".\server\"))
            {
